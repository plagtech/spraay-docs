...
Because the export comes from `HEAD`, commit first; uncommitted edits are not deployed. After deploying, confirm `docs.spraay.app/CLAUDE.md` no longer returns markdown and that `/`, `/llms.txt` and `/bpa/1.0/` still serve. Note that the site has no `404.html`, so Pages answers unknown paths (including `/CLAUDE.md`) with `index.html` and HTTP 200 rather than 404; check the `Content-Type` (`text/html`, not `text/markdown`). Cloudflare may keep serving a previously cached copy of the old markdown at the edge for up to a week (`Cache-Control: s-maxage=604800`, visible via the `Age` header) unless the URL is purged from the spraay.app zone cache in the Cloudflare dashboard. Wrangler leaves a `.wrangler/` cache directory in the folder it runs from; delete it rather than committing it.
